...
const licenceFolder = join(process.cwd(), 'content', 'licences')
...
export async function getLicenceIds(_locale: Locale): Promise<Array<string>> {
  const ids = await readFolder(licenceFolder, licenceExtension)
...
export function getLicenceFilePath(id: ID, _locale: Locale): FilePath {
  const filePath = join(licenceFolder, id + licenceExtension)
